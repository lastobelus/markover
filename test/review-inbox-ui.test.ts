...
test('production inbox shares responsibility filters and retains All history', () => {
  ...
  assert.match(main, /omittedCount > 0[\s\S]*Showing the latest 50;[\s\S]*earlier.*omitted/)
  assert.match(main, /const cached = await readCanonicalUpdateManifestCache[\s\S]*if \(cached\) \{[\s\S]*void refreshCanonicalUpdateManifestCache\(\)[\s\S]*return canonicalUpdateManifestLiveCheckFailed \? null : cached/)
  assert.match(main, /canonicalUpdateManifestLiveCheckFailed = true[\s\S]*return null/)
  ...
  assert.match(renderer, /canonicalUpdateStatus\.state !== 'available' &&[\s\S]*canonicalUpdateStatus\.state !== 'unavailable' &&[\s\S]*canonicalUpdateStatus\.state !== 'unknown'/)
