...
test('capture treats numeric extension leaves as private artifact values', async (t) => {
  ...
  await t.test('whitespace-folded Base64 private values', () => {
    const artifact = fixture()
    agentThread(artifact)
    const rootNode = artifact.root as Record<string, unknown>
    rootNode.fixtureExtension = { secret: 'YWNjdD\r\nEyMzQ1' }
    const runtime = observation().runtime as Record<string, unknown>
    runtime.providerModel = 'acct12345'
    assert.throws(
      () => buildSanitizedEvidence(
  ...
  await t.test('private values reconstructed from multi-segment decoded alternatives', () => {
    const artifact = fixture()
    agentThread(artifact)
    const rootNode = artifact.root as Record<string, unknown>
    rootNode.fixtureExtension = { secret: 'acct12345' }
    const runtime = observation().runtime as Record<string, unknown>
    runtime.providerModel = 'YWNjdCAxMg MzQ1'
    assert.throws(
      () => buildSanitizedEvidence(
        artifact,
        observation({ runtime }),
  ...
  await t.test('alphabetic-leading base-36 private numeric identities', () => {
    const artifact = fixture()
    agentThread(artifact)
    const rootNode = artifact.root as Record<string, unknown>
    rootNode.fixtureExtension = { accountId: 1679615 }
    const runtime = observation().runtime as Record<string, unknown>
    runtime.providerModel = 'zzzz'
    assert.throws(
      () => buildSanitizedEvidence(
        artifact,
        observation({ runtime }),
  ...
  await t.test('full-width base-36 private numeric identities', () => {
    const artifact = fixture()
    agentThread(artifact)
    const rootNode = artifact.root as Record<string, unknown>
    rootNode.fixtureExtension = { accountId: '170581728179578208255' }
    const runtime = observation().runtime as Record<string, unknown>
    runtime.providerModel = 'zzzzzzzzzzzzz'
    assert.throws(
      () => buildSanitizedEvidence(
        artifact,
        observation({ runtime }),
  ...
  await t.test('Base58btc encoded private identities', () => {
    const artifact = fixture()
    agentThread(artifact)
    const rootNode = artifact.root as Record<string, unknown>
    rootNode.fixtureExtension = { accountId: 'acct12345' }
    const runtime = observation().runtime as Record<string, unknown>
    runtime.providerModel = '2Eu6hXQuJGc3n'
    assert.throws(
      () => buildSanitizedEvidence(
        artifact,
        observation({ runtime }),
  ...
  await t.test('multibase-prefixed Base58btc private identities', () => {
    ...
    rootNode.fixtureExtension = { accountId: 'acct12345' }
    const runtime = observation().runtime as Record<string, unknown>
    runtime.providerModel = 'z2Eu6hXQuJGc3n'
    assert.throws(
      () => buildSanitizedEvidence(
        artifact,
        observation({ runtime }),
  ...
  await t.test('canonical Base32hex private identities', () => {
    const artifact = fixture()
    agentThread(artifact)
    const rootNode = artifact.root as Record<string, unknown>
    rootNode.fixtureExtension = { accountId: 'acct12345' }
    const runtime = observation().runtime as Record<string, unknown>
    runtime.providerModel = 'C5HM6T1H68PJ8D8'
    assert.throws(
      () => buildSanitizedEvidence(
        artifact,
        observation({ runtime }),
  ...
  await t.test('unprefixed Proquint private values', () => {
    const artifact = fixture()
    agentThread(artifact)
    const rootNode = artifact.root as Record<string, unknown>
    rootNode.fixtureExtension = { secret: 'azR1' }
    const runtime = observation().runtime as Record<string, unknown>
    runtime.providerModel = 'kajup-jamud'
    assert.throws(
      () => buildSanitizedEvidence(
        artifact,
        observation({ runtime }),
  ...
  await t.test('multibase-prefixed base-8 private numeric identities', () => {
    const artifact = fixture()
    agentThread(artifact)
    const rootNode = artifact.root as Record<string, unknown>
    rootNode.fixtureExtension = { accountId: 12345678 }
    const runtime = observation().runtime as Record<string, unknown>
    runtime.providerModel = '757060516'
    assert.throws(
      () => buildSanitizedEvidence(
        artifact,
        observation({ runtime }),
  ...
  await t.test('multibase-prefixed base-2 private numeric identities', () => {
    const artifact = fixture()
    agentThread(artifact)
    const rootNode = artifact.root as Record<string, unknown>
    rootNode.fixtureExtension = { accountId: 42 }
    const runtime = observation().runtime as Record<string, unknown>
    runtime.providerModel = '0101010'
    assert.throws(
      () => buildSanitizedEvidence(
        artifact,
        observation({ runtime }),
  ...
  await t.test('multibase Base32z private identities', () => {
    const artifact = fixture()
    agentThread(artifact)
    const rootNode = artifact.root as Record<string, unknown>
    rootNode.fixtureExtension = { accountId: 'acct12345' }
    const runtime = observation().runtime as Record<string, unknown>
    runtime.providerModel = 'hcftsg7btge3uepe'
    assert.throws(
  ...
  await t.test('ASCII85 encoded private values', () => {
    const artifact = fixture()
    agentThread(artifact)
    const rootNode = artifact.root as Record<string, unknown>
  ...
  await t.test('whitespace-separated Adobe ASCII85 private values', () => {
    const artifact = fixture()
    agentThread(artifact)
    const rootNode = artifact.root as Record<string, unknown>
    rootNode.fixtureExtension = { secret: '<~@:Neh\n0etOA2#~>' }
    const runtime = observation().runtime as Record<string, unknown>
    runtime.providerModel = 'acct12345'
    assert.throws(() => buildSanitizedEvidence(artifact, observation({ runtime }),
      json('evals/review-metadata/matrix.json')), /runtime still contains a private artifact value/)
  ...
  await t.test('Z85 encoded private values', () => {
    const artifact = fixture()
    agentThread(artifact)
    const rootNode = artifact.root as Record<string, unknown>
    rootNode.fixtureExtension = { secret: 'abcd' }
    const runtime = observation().runtime as Record<string, unknown>
    runtime.providerModel = 'vpA.S'
    assert.throws(() => buildSanitizedEvidence(artifact, observation({ runtime }),
      json('evals/review-metadata/matrix.json')), /runtime still contains a private artifact value/)
  })

  await t.test('quoted-printable private values', () => {
    const artifact = fixture()
    agentThread(artifact)
    const rootNode = artifact.root as Record<string, unknown>
    rootNode.fixtureExtension = { secret: '=61=63=63=74=31=32=33=34=35' }
    const runtime = observation().runtime as Record<string, unknown>
    runtime.providerModel = 'acct12345'
    assert.throws(() => buildSanitizedEvidence(artifact, observation({ runtime }),
      json('evals/review-metadata/matrix.json')), /runtime still contains a private artifact value/)
  ...
  await t.test('semicolonless HTML numeric character-reference private values', () => {
    const artifact = fixture()
    agentThread(artifact)
    const rootNode = artifact.root as Record<string, unknown>
    rootNode.fixtureExtension = {
      secret: '&#97&#99&#99&#116&#49&#50&#51&#52&#53'
    }
    const runtime = observation().runtime as Record<string, unknown>
    runtime.providerModel = 'acct12345'
    assert.throws(
  ...
  await t.test('backtick-padded uuencoded private values', () => {
    const artifact = fixture()
    agentThread(artifact)
    const rootNode = artifact.root as Record<string, unknown>
    rootNode.fixtureExtension = { secret: '$86)C9```' }
    const runtime = observation().runtime as Record<string, unknown>
    runtime.providerModel = 'abcd'
    assert.throws(
      () => buildSanitizedEvidence(artifact, observation({ runtime }),
        json('evals/review-metadata/matrix.json')),
      /runtime still contains a private artifact value/
  ...
  await t.test('boolean additive scalar values fail closed', () => {
    const artifact = fixture()
    agentThread(artifact)
    const rootNode = artifact.root as Record<string, unknown>
    rootNode.fixtureExtension = { secret: true }
    const runtime = observation().runtime as Record<string, unknown>
    runtime.providerModel = 'true'
    assert.throws(
      () => buildSanitizedEvidence(artifact, observation({ runtime }),
        json('evals/review-metadata/matrix.json')),
      /additive scalar independence cannot be proven/
  ...
  await t.test('basE91 encoded known private values', () => {
    const artifact = fixture()
    agentThread(artifact)
    const review = artifact.review as Record<string, unknown>
    review.contextSummary = 'uJ(I,i~RR2a'
    const runtime = observation().runtime as Record<string, unknown>
    runtime.providerModel = 'acct12345'
    assert.throws(
      () => buildSanitizedEvidence(artifact, observation({ runtime }),
