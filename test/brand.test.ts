...
test('the working header aligns the brand and uses the primary action color', () => {
  ...
  assert.match(styles, /\.document-tab-status\.is-pending-agent \{[^}]*background: var\(--brand-burgundy\);/)
  assert.match(styles, /\.document-tab-status\.is-revised \{[^}]*background: var\(--status-revised\);/)
  assert.match(styles, /\.document-tab-status\.is-done \{[^}]*color: var\(--paper\);[^}]*background: var\(--status-done\);/)
  assert.match(styles, /\.document-tab\.is-active \{[^}]*color: var\(--brand-orange\);/)
  assert.match(styles, /\.document-tab-shell \+ \.document-tab-shell \{[^}]*border-left: 1px solid var\(--surface\);/)
  assert.match(styles, /\.document-tab-shell \{[^}]*max-width: 320px;/)
  assert.match(styles, /\.document-tab \{[^}]*justify-content: center;/)
