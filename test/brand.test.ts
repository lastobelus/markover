...
test('the working header aligns the brand and exact review identity', () => {
  ...
  assert.match(styles, /\.app-header-bar \{[^}]*min-height: 67px;[^}]*padding: 10px 22px 8px;/)
  ...
  assert.match(styles, /\.review-navigation-tabs \{[^}]*gap: 6px;[^}]*padding-inline: 8px;/)
  ...
  assert.match(styles, /\.pane-view-tabs > button \{[^}]*color: var\(--pane-label-inactive\);/)
  ...
  assert.match(styles, /\.pane-view-tabs > button\.is-active::after \{[^}]*height: 1px;[^}]*background: currentColor;/)
