...
test('release workflow publishes both native architectures and the tiny CLI', () => {
  ...
  assert.match(workflow, /rollback-arm64-tag/)
  assert.match(workflow, /rollback-x64-tag/)
  assert.match(workflow, /rollback-arm64-fingerprint/)
  assert.match(workflow, /rollback-x64-fingerprint/)
...
test('developer release runbook preserves provenance and rollback boundaries', () => {
  ...
  assert.match(runbook, /one target per native architecture/i)
  ...
  assert.match(runbook, /v0\.1\.4.*unchanged and unpublished/i)
...
test('ordinary workflows use Node 24 and release candidates cover Node 22', () => {
  ...
  assert.match(decisions, /required `Verify \(Node 24\)` CI check/)
  assert.match(
    decisions,
    /tag-triggered release workflow tests that exact candidate on the\s+minimum-supported Node 22\.13\.0/
  )
  assert.doesNotMatch(decisions, /both required CI checks/)
