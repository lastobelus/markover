...
| ChatGPT desktop with Codex | ChatGPT `26.810.52044` (build `6662`); bundle ID `com.openai.codex` | Installed as `/Applications/ChatGPT.app`; runtime exposure is not yet established. [Official OpenAI documentation](https://developers.openai.com/) confirms that ChatGPT and Codex now share one app. | Checkpoint 5 must exercise the installed ChatGPT/Codex desktop surface and record its exact-ID or exact-key capability; do not look for a legacy `Codex.app` bundle. |
