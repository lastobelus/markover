...
| Claude desktop | Claude.app `1.30096.5` | A live handoff capture recovered the exact persisted Claude session, and the agent reports a nonblank `CLAUDE_CODE_SESSION_ID`; the GUI does not expose that ID directly. | Handoff passes; checkpoint 5 must also record the now-proven explicit runtime route. |
