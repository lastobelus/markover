...
Recorder-presence provenance enforcement in runner commit
`904f8ed301291371bd3ad7effa4af2499b83267f` produced a one-hundred-sixteenth
record for each combination, using independent suffix `hqzvnjki`.
...
`issue-136-runner-history-v100` tag, so a clean checkout can inspect any recorded
runner with:

```sh
git fetch origin refs/tags/issue-136-runner-history-v100
