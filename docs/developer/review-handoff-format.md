...
| `review.status` | Exactly `editing`, `pending-agent`, `revised`, or `done`. `editing` is mutable; the other states are read-only. `done` is terminal and requires a non-null pull request with a stored `merged` observation. Another feedback round creates an independent review. |
...
| `review.agentThread.threadHost` | Requires nonblank open-string `kind` and `provider`. They are separate dimensions but may have the same value. Optional nonblank `threadId` is present only for a distinct thread-host identifier and must not equal `agentThread.id`; omission means consumers fall back to `agentThread.id`. Optional nonblank `machine` is an agent-reported hostname snapshot, descriptive and never identity-bearing. |
...
require a new version for a new value.
