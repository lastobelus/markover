...
| `review.agentThread` | Required nullable request snapshot. When non-null it requires nonblank provider-owned `id` and a `threadHost` object. App-private session evidence is forbidden at both levels: `cwd`, `logPath`, `discovery`, `parentThreadId`, `forkedFromId`, and the requesting-thread-title aliases `title`, `name`, and `requestingThreadTitle`. |
