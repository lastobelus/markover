...
- `agentThread.threadHost.machine`: the exact machine string the requesting agent reports at request time, such as `canonical-host.local`. It is descriptive, may change between packets, and is never durable identity. Markover cannot reconcile a machine rename from this field alone.
