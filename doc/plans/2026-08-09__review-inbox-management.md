...
- `threadHost.kind` identifies the concrete thread-host application, such as `t3code`, `lastcode`, or `codex-app`. `threadHost.machine` is the machine string reported for that request, such as `canonical-host.local`; it is descriptive context rather than durable identity, and a later rename cannot be reconciled from that value alone.
