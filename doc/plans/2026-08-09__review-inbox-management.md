...
3. Identify the right review: show recognizable current thread titles and stable project/document context.
...
- Viewing a review, annotation autosaves, title refreshes, and document-tab activation do not reorder Inbox.
...
2. Prominent: current user-visible thread title, preceded by the provider icon.
3. Small: branch, with pull request right-aligned when available.

The project favicon leads the row. Missing favicon/provider/branch/PR values use neutral fallbacks without collapsing the document or title identity. Long values truncate independently and expose their full accessible label.
...
- Display the current user-visible thread title, including user renames, rather than a permanent snapshot of the original generated title.
- Resolve title authority in this order: host-visible title, provider title, then a clearly labeled fallback such as context summary or document name.
- Never infer a current title from the opening prompt or stale preview.
- Preserve title source/provenance and observation time.
- Refresh when a review arrives, when Markover launches or returns to foreground, and when Inbox or Projects opens. Provide a manual `Refresh Title` action.
- If an integration is temporarily unavailable, retain the last authoritative title rather than replacing it with a weaker fallback.
...
- Tool-assisted title discovery must request the current user-visible title and evidence source, prohibit inference from original prompts/previews, and return unavailable rather than guess.
...
- current thread title, authority/provenance, and observation time;
...
- `src/contracts.ts`: persisted review-origin, recency, title-provenance, project-identity, and workspace-state contracts.
...
- `src/metadata-discovery.ts`: repository identity plus host/provider title adapters.
...
- Inbox ordering changes only when `attentionRequestedAt` changes; unrelated autosaves, viewing, title refreshes, and tab actions do not reorder it.
...
- Agent and Local rows remain distinguishable and useful with duplicate thread titles, timestamp-prefixed filenames, missing metadata, and long values.
...
- Disabled integrations perform no metadata inspection; invalid overrides explain the failure and do not destroy the last authoritative title.
...
- Integration-test store → main/preload → renderer flows for status changes, title refresh, restart restoration, deep-link ancestor reveal, tab closure, and Local opens.
- Test explicit integration enablement, path overrides, unavailable sources, stale authoritative-title retention, and completed rename experiments.
...
- Additional host/provider adapters follow completed rename experiments rather than speculative shared parsing.
