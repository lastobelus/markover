...
| `review.agentThread` | Required nullable typed request snapshot. When non-null, it requires nonblank provider-owned `id` and a `threadHost` object. No requesting-thread-title observation belongs here. |
| `review.agentThread.threadHost` | Requires nonblank open-string `kind` and `provider`; they are separate dimensions but may have the same value. Optional nonblank `threadId` is included only when a distinct thread-host identifier exists and must not equal `agentThread.id`; omission means consumers fall back to `agentThread.id`. Optional nonblank `machine` is the agent-reported hostname snapshot, descriptive and never identity-bearing. |
